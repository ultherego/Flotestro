...
func TestClassifyFollowsTheTable(t *testing.T) {
	...
	cases := []struct {
		...
		{"the last attempt is exhausted", status(500), 20, StateDeadLetter, CodeAttemptsExhausted},
		{"a refusal on the last attempt is exhausted", SendError{Code: CodeConnectionRefused, Err: errors.New("refused")}, 20, StateDeadLetter, CodeAttemptsExhausted},
