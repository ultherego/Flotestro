...
func (s *Server) writeSysctl(ctx context.Context, action *helperv1.KernelRequest) *helperv1.HelperResponse {
	...
	for key := range settings {
		...
		if _, err := toolOutput(ctx, kernel.SciezkaSysctl, "-n", key); err != nil {
	...
	if content, err := os.ReadFile(kernel.PlikSysctl); err == nil {
		current = kernel.ParsujPlikSysctl(string(content))
	}
	for key, value := range settings {
		current[key] = value
	}
	content, err := kernel.SkladajPlikSysctl(current)
	if err != nil {
		return reject(ErrorMalformed, err.Error())
	}
	if err := writeKernelFile(kernel.PlikSysctl, content, 0o644); err != nil {
		return reject(ErrorExecFailed, "writing "+kernel.PlikSysctl+": "+err.Error())
	}
	...
	for key, value := range settings {
		if _, err := runTool(ctx,
			[]string{kernel.SciezkaSysctl, "-w", key + "=" + value}); err != nil {
			pending = append(pending, key+" = "+value)
			continue
		}
		now, err := toolOutput(ctx, kernel.SciezkaSysctl, "-n", key)
...
func (s *Server) loadModule(ctx context.Context, action *helperv1.KernelRequest) *helperv1.HelperResponse {
	...
	output, err := runTool(ctx, []string{kernel.SciezkaModprobe, action.GetModule()})
	if err != nil {
...
func (s *Server) blockModule(ctx context.Context, action *helperv1.KernelRequest) *helperv1.HelperResponse {
	...
	if expected := action.GetPlanHash(); expected != "" {
		now := kernel.ZaplanujBlokade(s.readKernel(ctx, nil), action.GetModule(), action.GetBlacklist())
	...
	current := []string{}
	if content, err := os.ReadFile(kernel.PlikBlacklisty); err == nil {
		current = kernel.ParsujBlacklist(string(content))
	}
	...
	content, err := kernel.SkladajBlacklist(updated)
	if err != nil {
		return reject(ErrorMalformed, err.Error())
	}
	if err := writeKernelFile(kernel.PlikBlacklisty, content, 0o644); err != nil {
		return reject(ErrorExecFailed, "writing "+kernel.PlikBlacklisty+": "+err.Error())
	}
	...
	if action.GetBlacklist() {
		...
		if reason := kernel.InitramfsWymagany(action.GetModule(), s.moduleLoaded(action.GetModule())); reason != "" {
			message += "; " + reason
		}
	}
...
func (s *Server) planBlock(ctx context.Context, action *helperv1.KernelRequest) *helperv1.HelperResponse {
	...
	plan := kernel.ZaplanujBlokade(state, action.GetModule(), action.GetBlacklist())
...
func (s *Server) readKernel(ctx context.Context, extra []string) kernel.Snapshot {
	...
	if data, err := os.ReadFile("/proc/modules"); err == nil {
		snapshot.Modules = kernel.ParsujModuly(string(data))
	}

	written := map[string]string{}
	if content, err := os.ReadFile(kernel.PlikSysctl); err == nil {
		snapshot.Managed = string(content)
		written = kernel.ParsujPlikSysctl(string(content))
	}
	if content, err := os.ReadFile(kernel.PlikBlacklisty); err == nil {
		snapshot.Blacklist = kernel.ParsujBlacklist(string(content))
	}
	...
	// answer.
	keys := append([]string{}, kernel.ProfilDomyslny...)
...
func (s *Server) readSettings(ctx context.Context, keys []string,
	written map[string]string) []kernel.Ustawienie {
	seen := map[string]bool{}
	var result []kernel.Ustawienie
	for _, key := range keys {
		if seen[key] || kernel.WalidujKlucz(key) != nil {
			continue
		}
		seen[key] = true
		setting := kernel.Ustawienie{Key: key}
		if value, ok := written[key]; ok {
			setting.Desired = value
			setting.Managed = true
			setting.Source = kernel.PlikSysctl
		}
		if output, err := toolOutput(ctx, kernel.SciezkaSysctl, "-n", key); err == nil {
...
func (s *Server) moduleLoaded(name string) bool {
	...
	for _, module := range kernel.ParsujModuly(string(data)) {
		if module.Name == name {
			return true
		}
	}
