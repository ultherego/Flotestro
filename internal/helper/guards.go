...
// mutation of a class runs at a time; a read takes no guard.
const (
	GuardUnits      = "units"
	...
	// The network is one resource: NetworkManager profiles, the resolver and the
	// firewall table are rewritten as a whole, and two changes at once leave a
	// state neither rollback plan describes.
	...
	// and a second operation would wait under that lock until its time ran out -
	// without the panel knowing why.
	GuardBackup = "backup"
...
// guards serialize the mutations on the resources of the host. A guard is
// refused, not waited for.
type guards struct {
	mu sync.Mutex
	// held maps a class to the task holding it.
...
// busyPrefix opens every refusal of a held guard.
...
// deadline derives the context of one operation from the order.
