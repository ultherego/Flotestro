...
// RenewRunner extends the lease the runner holds.
...
// ReleaseRunner gives the lease back when the orchestrator stops, so the next
// instance takes the campaign at once rather than after the term.
...
// AdoptTargets puts every open target of the campaign under the runner's claim
// and renews the claims it holds already.
func (s *Store) AdoptTargets(ctx context.Context, campaignID, runner string) error {
	...
	// The claims renewed are those of the hosts carrying a task: a few rows per
	// campaign.
	if _, err := tx.Exec(ctx, `
...
// runner, in rollout order, and returns them with the revision and the token
// the rows carry now.
...
// checks that the campaign may start a host now, under the lease the runner
// holds.
