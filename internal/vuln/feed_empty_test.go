...
// An empty fetch never replaces a snapshot that carried findings: the vendors
// do not fix everything at once, and a feed that went from thousands to zero
...
// The refusal is typed so the scheduler can mark the source with the reason
// rather than log an unrecognised failure, and the reason it writes is the
// code the panel shows next to a stale feed.
