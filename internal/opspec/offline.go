...
const (
	// OfflineRequireOnline ends the host's participation the moment it is found
	// offline: the order is reactive or contextual, and the state it was given
	// for will not be there when the host comes back.
	OfflineRequireOnline OfflinePolicy = "require_online"
	// OfflineSkip leaves the host out without calling that a failure: a read or a
	// refresh that did not happen is a gap in knowledge, not a broken change.
	...
	// OfflineReplan waits like OfflineWait, but computes the host's plan again
	// after the reconnect and runs the change only when the plan is the one that
	// was approved.
...
// strictness orders the policies by how little they do with an offline host.
...
// offlinePolicies is the registry of explicit offline policies. The map lists
// the operations whose policy does not follow from their shape.
var offlinePolicies = map[ActionType]OfflinePolicy{
	// Reactive and contextual orders.
	...
	// tomorrow, so the host gets it when it comes back, within the campaign's
	// deadline.
...
// ResolveOfflinePolicy returns the policy a campaign runs under. An empty
// request means the operation's own policy.
