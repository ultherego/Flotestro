...
var reportedGuides = []ErrorGuide{
	...
	{Code: "cancel_ack_stale", Stage: "dispatch", Retry: RetryAutomatic,
		Meaning: "The host answered a cancel request the panel had already replaced.",
		Action:  "Nothing: the answer is on the trail and the current request is still outstanding."},
	{Code: "crypto_state_stale", Stage: "startup", Retry: RetryAutomatic,
		Meaning: "This control-plane instance's key material is older than the installation record and it could not catch up.",
		Action:  "The instance answers queries and signs with what it has, so it is taken out of the rotation until it reloads. Read its log: the key or the authority the record names is not readable here."},
	...
	{Code: "migration_checksum_mismatch", Stage: "startup", Retry: RetryNever,
		Meaning: "A migration this build carries is not the one the database records under that version.",
		Action:  "The same version label describes two schemas. Nothing was migrated and the control plane will not serve; start the build whose migrations match, or reconcile the schema by hand."},
	...
	{Code: "inhibitors_unknown", Stage: "helper", Retry: RetryAfterChange,
		Meaning: "The host could not be asked whether anything is holding a restart or a shutdown back.",
		Action:  "Read the host: systemd-inhibit may be missing or its output unrecognised. Order again with the inhibitor override to go ahead regardless."},
	...
	{Code: "output_too_large", Stage: "agent", Retry: RetryAfterChange,
		Meaning: "The result of the operation does not fit the limit the task declared. A cut JSON document is no document at all, so the whole result is refused rather than trimmed to fit.",
		Action:  "Narrow what was asked for - one unit rather than every unit, a shorter window - or raise max_output_bytes on the operation. The message names both the size and the limit."},
	{Code: "tool_output_too_large", Stage: "agent", Retry: RetryAfterChange,
		Meaning: "A tool of the host wrote more than the agent reads in one go, so the answer is only part of what it said. A listing read in part would name fewer packages, fewer rules or fewer failed units than the host has, and fewer is not a smaller truth - so nothing was read out of it.",
		Action:  "Look at what that tool prints on the host: a repository that answers with a page of errors, or a database left in a state the tool describes at length. Fix that, then order again.", CountsAsFailure: true},
	...
	{Code: "trust_bundle_invalid", Stage: "helper", Retry: RetryNever,
		Meaning: "The trust bundle the agent handed the helper is not a bundle: it names no host, carries no key, or a key in it is named after another key.",
		Action:  "The bundle was altered on the way; inspect the host and the path the session takes to it.", CountsAsFailure: true},
	{Code: "trust_bundle_untrusted", Stage: "helper", Retry: RetryNever,
		Meaning: "The trust bundle is signed by a key this helper does not already trust, or its signature does not verify.",
		Action:  "Let the agent reconnect so the panel's current bundle reaches the host; a host whose keyring was lost is reset by hand with flotestro-agentctl helper-trust reset.", CountsAsFailure: true},
	{Code: "trust_pin_mismatch", Stage: "helper", Retry: RetryNever,
		Meaning: "The first trust bundle this host was handed is signed by a key the host does not name in its pin file, or the host names none and enrolls with nobody.",
		Action:  "Check which panel the host was meant to join; write that panel's signing fingerprint with flotestro-agentctl helper-trust pin, and order the enrollment again.", CountsAsFailure: true},
	...
	{Code: "payload_binding_unchecked", Stage: "helper", Retry: RetryNever,
		Meaning: "The helper holds no rule for comparing this kind of request with the payload the capability binds, so it refuses rather than carry out a change nothing compared.",
		Action:  "Nothing on the host is wrong: the helper was built with an operation whose binding rule was never written. Report it and use another operation meanwhile.", CountsAsFailure: true},
	{Code: "capability_in_flight", Stage: "helper", Retry: RetryAutomatic,
		Meaning: "The very same request is already being carried out under this capability; asking again does not start it a second time.",
		Action:  "Wait for the answer of the request that is running; it is the answer to this one.", CountsAsFailure: false},
	{Code: "capability_already_performed", Stage: "helper", Retry: RetryNever,
		Meaning: "The very same request already ran under this capability and its answer was too large for the host to keep, so the helper can neither repeat the answer nor repeat the effect.",
		Action:  "Read the state of the host to see what the operation left behind; it happened once and only once.", CountsAsFailure: true},
	...
	{Code: "idempotency_key_reused", Stage: "admission", Retry: RetryAfterChange,
		Meaning: "The idempotency key of this order was already used on this host for a different operation.",
		Action:  "The first task stands and is named in the answer; order again with a key of this order's own."},
	...
	// 10).
	{Code: "account_privilege_unknown", Stage: "admission", Retry: RetryAfterChange,
		Meaning: "The order would hand somebody the access an account already has - a key added, the key list rewritten, a lock or an expiry taken off - and the host has not reported which groups that account belongs to. Whether the account is root by another name could not be established, and a fact nobody could read is not a favourable one, so nothing was ordered.",
		Action:  "Wait for the host's next inventory report, or have it refreshed, and order again; the refusal names the account. An order that must go now is placed by a principal holding accounts.privileged_groups."},
	{Code: "channel_address_not_allowed", Stage: "notification", Retry: RetryAfterChange,
		Meaning: "The address of a notification channel is not one the panel sends to: its own loopback, a link-local address where a cloud metadata service answers, a private or reserved address inside the installation's own network, or a receiver that redirects to another host. Nothing was sent.",
		Action:  "Point the channel at the receiver's own address. A receiver that genuinely sits on this installation's network is declared by listing its range in FLOTESTRO_NOTIFY_ALLOW on the control plane, comma-separated; then retry the dead letters."},
	{Code: "channel_left_behind", Stage: "notification", Retry: RetryAfterChange,
		Meaning: "A notification channel was left on record that nothing backs: its credential was not stored and the panel could not undo the row either, or could not switch the channel off.",
		Action:  "Look at the channel the message names: remove it if it is new, or set its credential again if it existed before. The database or the secret store was answering badly, so check both.", CountsAsFailure: true},
	{Code: "channel_left_disabled", Stage: "notification", Retry: RetryAfterChange,
		Meaning: "The change to the notification channel is stored and its credential is not, so the channel was switched off rather than left sending under a credential that no longer matches it.",
		Action:  "Set the credential again and switch the channel on; nothing was sent under the wrong one.", CountsAsFailure: true},
	{Code: "webhook_secret_required", Stage: "notification", Retry: RetryAfterChange,
		Meaning: "A webhook channel has no signing secret. An empty key is still a legal HMAC key, so a delivery signed with it carries a header anybody can compute, and a receiver that verifies the header when it is present would take a forged body.",
		Action:  "Set a signing secret on the channel, give the same secret to the receiver, and retry the dead letters. A receiver that cannot verify a signature belongs behind a channel of another kind."},
	...
	{Code: "plan_binding_missing", Stage: "admission", Retry: RetryAfterChange,
		Meaning: "The operation declares that it must be ordered from an approved plan, and the order carried nothing binding it to one. Nothing was sent to the host: a package change resolved at the moment it runs is not the change anybody approved.",
		Action:  "Order the plan first, read what it would do, and send the change with the hash the plan returned. The panel does this by itself; an API caller has to ask for the plan and pass its hash."},
	{Code: "security_metadata_unavailable", Stage: "agent", Retry: RetryAfterChange, CountsAsFailure: true,
		Meaning: "The host cannot tell a security update from any other: its repositories publish no advisory for the pending updates. Nothing was planned and nothing was upgraded - a security-only job that filtered them away would have reported the host as patched.",
		Action:  "Plan a full upgrade, or narrow the order to packages whose repository publishes advisories. No Arch repository does, so a security-only job is never possible there."},
	{Code: "partial_upgrade_unsupported", Stage: "agent", Retry: RetryAfterChange, CountsAsFailure: true,
		Meaning: "The host's package manager upgrades the whole system or nothing. Upgrading a named subset would leave the installation in a combination its distribution never tests, which is how an Arch host is broken.",
		Action:  "Order the upgrade without naming packages, so the host takes the whole transaction its distribution expects."},
	{Code: "schedule_adopt_shared_file", Stage: "helper", Retry: RetryAfterChange, CountsAsFailure: true,
		Meaning: "The entry to adopt shares its file with other lines - a distribution's cron file holds one job per line. The panel takes a line over by removing the file it is alone in, so adopting this one would have removed entries nobody ordered it to touch. Nothing was written and nothing was removed.",
		Action:  "Read the file the refusal names, take that one line out by hand, and order the entry again; or give the entry another name."},
	...
	{Code: "support_bundle_not_ready", Stage: "admission", Retry: RetryAutomatic,
		Meaning: "The bundle is still being assembled, so there is nothing to hand over yet.",
		Action:  "Wait until the screen says it is ready; the link is issued then."},
	{Code: "support_bundle_not_found", Stage: "admission", Retry: RetryNever,
		Meaning: "There is no such bundle. A bundle is deleted when its retention runs out, and one nobody fetched goes sooner than one somebody did.",
		Action:  "Ask for a new bundle; an old one is not kept for the sake of keeping it."},
	{Code: "support_bundle_unsealed", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "This panel holds no key material, so it can neither seal a new bundle nor open one that is sealed. A bundle readable without a key is not one this panel writes.",
		Action:  "Bring the installation's key provider back - the key file or the credential the panel was started with - and ask again."},
	{Code: "support_bundle_failed", Stage: "reconcile", Retry: RetryAfterChange,
		Meaning: "The bundle could not be assembled; the readings that failed name their own reason in the log of the panel.",
		Action:  "Read the panel log for this bundle and ask for another one."},
	{Code: "support_bundle_abandoned", Stage: "reconcile", Retry: RetryAfterChange,
		Meaning: "The panel that was assembling this bundle went away before it finished. A row still in progress past the assembly budget is said to have failed rather than left looking like work.",
		Action:  "Ask for a new bundle."},
	{Code: "support_bundle_token_required", Stage: "approval", Retry: RetryAfterChange,
		Meaning: "The archive was asked for without the link the panel issues. A bundle is not fetched by naming its identifier.",
		Action:  "Ask for a download link on the support screen and follow it."},
	{Code: "support_bundle_token_invalid", Stage: "approval", Retry: RetryAfterChange,
		Meaning: "This link is not one this panel issued, or the bundle behind it is gone.",
		Action:  "Ask for a new link; if the bundle is gone, ask for a new bundle."},
	{Code: "support_bundle_token_expired", Stage: "approval", Retry: RetryAfterChange,
		Meaning: "The link has run out. It lasts minutes on purpose: a link that keeps working is a copy of the bundle in whatever it was pasted into.",
		Action:  "Ask for a new link and follow it straight away."},
	{Code: "support_bundle_token_spent", Stage: "approval", Retry: RetryAfterChange,
		Meaning: "The link has already been followed. One link hands the archive over once.",
		Action:  "Ask for a new link."},
	{Code: "support_bundle_token_foreign", Stage: "approval", Retry: RetryNever,
		Meaning: "The link was issued to another identity. A forwarded link does not carry the right to fetch with it.",
		Action:  "Ask for a link of your own; the permission and the fresh authentication are checked for whoever fetches."},
	{Code: "support_bundle_token_mismatch", Stage: "approval", Retry: RetryAfterChange,
		Meaning: "The link opens a different bundle than the one it was pointed at.",
		Action:  "Follow the link exactly as the panel issued it."},
