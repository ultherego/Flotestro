...
var reportedGuides = []ErrorGuide{
	...
	{Code: "plan_metadata_missing", Stage: "agent", Retry: RetryAfterChange,
		Meaning: "The host has no repository metadata an upgrade plan could be read from: no copy of the sync database and a system database that was never synced. Nothing was planned - an empty plan would have looked like a host with nothing pending.",
		Action:  "Let the host refresh its metadata (order the plan again, which asks the helper for a sync), or sync it on the host; pacman-contrib is not needed.", CountsAsFailure: true},
	{Code: "versionlock_missing", Stage: "helper", Retry: RetryAfterChange,
		Meaning: "The host cannot hold a package version: dnf has no versionlock plugin. Nothing was attempted.",
		Action:  "Install python3-dnf-plugin-versionlock (dnf4) or dnf5-plugin-versionlock (dnf5) and refresh the capabilities, or drop the hold from the order.", CountsAsFailure: true},
	{Code: "account_tool_missing", Stage: "helper", Retry: RetryAfterChange,
		Meaning: "The shadow tool this account operation needs is not on the host; the message names it. Nothing was changed.",
		Action:  "Install the shadow tools of the distribution - passwd on Debian and Ubuntu, shadow-utils on Fedora and RHEL, shadow on Arch - and refresh the capabilities.", CountsAsFailure: true},
	{Code: "installation_state_mismatch", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "The state directory and the database are not one installation: they name two different ones, or one of them names none while the other has a history. The log says which of the two is the stranger.",
		Action:  "Never scale the control plane with separate state volumes against one database - two state directories mean two fleet CAs and two secret store keys for one installation. Give the replica the state directory of this installation, or point it at the database that belongs to the directory it has."},
	{Code: "gateway_id_in_use", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "Another control-plane instance is alive under this FLOTESTRO_GATEWAY_ID and is still renewing its heartbeat. Two replicas under one identifier are read as one instance by the job leases, the session fencing and the audit trail.",
		Action:  "Give this replica an identifier of its own - every replica has a unique FLOTESTRO_GATEWAY_ID and they share the database - or stop the instance the log names. A record of an instance that is gone is taken over by itself within a minute."},
	{Code: "schema_behind", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "The database misses migrations this binary carries, and this process does not bring the schema forward itself. A control plane serving a schema it does not match would answer with tables that are not there.",
		Action:  "Run the migrate command as its own job, with the credentials of the migrator, and start the serving replicas afterwards; schema-check answers whether it worked. A single-host installation may instead leave FLOTESTRO_AUTO_MIGRATE at true."},
	{Code: "schema_ahead", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "The database carries migrations this binary does not know: a newer control plane has already migrated it. Swapping the image back does not undo a schema.",
		Action:  "Deploy the version that migrated this database, or restore the backup of the database and of the state directory taken before the upgrade. Never delete rows from schema_migrations to make the two agree."},
	// The health of a relay (containerisation document, chapters 16 and 20).
	{Code: "relay_listener_unavailable", Stage: "startup", Retry: RetryAutomatic,
		Meaning: "The listener of the agents of a relay is closed or has refused every connection for longer than its grace period: the process runs but no longer serves its site.",
		Action:  "This is the one relay failure a restart answers. Read the detail for the cause - file descriptors, a port taken - before the restart, because it comes back otherwise."},
	{Code: "relay_upstream_unreachable", Stage: "admission", Retry: RetryAutomatic,
		Meaning: "A relay does not reach any gateway of the centre and is buffering into its spool. It is not ready to carry work and it is not broken.",
		Action:  "Repair the link to the centre. Do not restart the relay: it holds the results of the site, and they leave the spool only when the panel acknowledges them."},
	{Code: "relay_spool_critical", Stage: "admission", Retry: RetryAfterChange,
		Meaning: "The durable spool of a relay has grown into the reserve kept for control messages and job results, so the relay refuses new sessions before an existing record is lost.",
		Action:  "Restore the link to the centre so the spool drains, or raise spool.max_bytes on the relay if the site needs a longer outage covered."},
	{Code: "relay_spool_unwritable", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "The spool of a relay cannot reach its disk: a sync of the segment file failed. What the relay takes now is not what it holds after a power failure.",
		Action:  "Look at the filesystem of the state directory of the relay - full, read-only or failing - and at the disk under it. The relay reports itself not ready until a sync succeeds again."},
	{Code: "relay_certificate_expired", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "The certificate a relay presents has ended; the centre and the agents refuse it.",
		Action:  "Let the renewal succeed - it needs a reachable gateway - or enroll the relay again if the certificate is past recovery."},
	{Code: "relay_certificate_unknown", Stage: "startup", Retry: RetryAfterChange,
		Meaning: "A relay holds no end date for its own certificate, so it cannot say whether the certificate is valid and does not report itself ready on the strength of not knowing.",
		Action:  "Inspect the state directory of the relay; an identity without an end date means the saved material is incomplete and the relay has to be enrolled again."},
