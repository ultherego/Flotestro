...
// The reasons a capability could not be confirmed.
const (
	...
	// preserved account into is not there or cannot be read.
	ReasonPreserveContainerMissing = "preserve_container_missing"
	// ReasonModDNNotPermitted: the directory reported its effective rights and
	// the connector's service account may not move an entry.
	ReasonModDNNotPermitted = "moddn_not_permitted"
	// ReasonModDNRightsUnknown: the directory did not report entry level rights,
	// so whether the move is allowed could not be proven either way.
...
type DirectoryCapabilities struct {
	...
	// Subject is the account whose entry the rights were read on.
	Subject string `json:"subject,omitempty"`
	// Instruction is what an operator does about the first reason that matters.
...
func (c *Client) CapabilitiesFor(ctx context.Context, uid string) (DirectoryCapabilities, error) {
	...
	// The container of preserved accounts answers for itself: a directory that
	// lists it has it, and one that refuses the search has nowhere to move an
	// entry to.
...
// instructionFor says what lifts a reason.
...
// do: letters per attribute and, where the directory reports them, letters for
...
// sampleAccount names one account of the directory, for a preflight that has
// no operation to ask about.
